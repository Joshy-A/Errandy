...
def is_valid_password(password):
    password_regex = r'^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*()_+{}|":<>?`~])\S{7,}$'
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Invalid password. It must be at least 7 characters with one capital, one number, and one special character.', category='error')
        else:
